...
ec2-describe-snapshots|egrep "${DOW} [A-z0-9]+: Weekly Backup for $INSTANCE" > ${myHome}/doc/SnapshotsDOW.txt

# Create snapshot
ec2-detach-volume $VOLUME -i $INSTANCE -d $DEVICE
sleep 30
ec2-create-snapshot $VOLUME -d "${dateDOW}: Weekly Backup for ${INSTANCE} (VolID:$VOLUME})"

ec2-attach-volume $VOLUME -i $INSTANCE -d $DEVICE

# Delete snapshot for older backups with same day of week parameter
while read line
do 
   snapshot=`echo $line | awk '{print $2}'`
   echo deleting $snapshot
   ec2-delete-snapshot $snapshot
done < "${myHome}/doc/SnapshotsDOW.txt"
