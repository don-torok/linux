...
# -*- coding: utf-8 -*-
#
# Humedica specific code to create inpatient era
#
# PreConditions:
#   1) Empty inpatient_era table ( ptId, start_date, end_date, first_enc )
#   2) Select access to schema.encounter
#
# PostConditon:
#   inpatient_era table filled in with ptId, start_date, end_date, first_enc
#   
#
# Run string
#   inpatient_era.py [--help] [--debug] --user <user> --password <password> --schema <schema>
#                 --targetSchema [<target schema if different from source>]
#
# 
# Logic
# First create a table of all unique inpatient encounters where the visitId is not defined.
...
def usage():
            print( 'inpatient_era.py [--help] [--debug] --user <user> --password <password> --schema <schema>' \
                + ' --targetSchema [<target schema if different from source>]' \
...
def main( argv ):
    ...
    target_table = targetSchema + ".inpatient_era"
    ...
    try:
        ...
        sql = "CREATE TEMPORARY TABLE " + new_table_name  + newLine \
            ...
            + "   AND interaction_type = '" +  inpatient +"'" + newLine \
...
#
        ...
        sql = "CREATE temporary table first_era_encounter " + newLine \
            ...
            + "         WHERE interaction_type = '" +  inpatient + "'" + newLine \
            + "      )" + newLine \
            + " WHERE rowNumber = 1"
